...
const IssueRenew = () => {
    ...
    const fetchStartDateEndDate = async (sessionFromDt, currentDate) => {
        try {
            const response = await fetch(`${BaseURL}/api/issue/renew-all?startDate=${sessionFromDt}&endDate=${currentDate}`, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`
                }
            });
    ...
    const handleSubmit = async (event) => {
        ...
        const payload = {
            invoiceNo: issueReturnNumber,
            invoiceDate: formatDateToDDMMYYYY(issueReturnDate),
            generalMemberId: generalMember.find(member => member.memberId === parseInt(selectedMemberId))?.memberId,
            bookDetails: selectedBookDetails.map(row => ({
        ...
        try {
            const response = await fetch(`${BaseURL}/api/issue/book-renew`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="main-content">
                        ...
                        <Table striped bordered hover>
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>Member Name</th>
                                    <th>Issue Renew No</th>
                                    <th>Issue Renew Date</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {currentData.map((value, index) => (
                                    <tr key={value.stock_id}>
                                        ...
                                        <td>
                                            <Eye className="ms-3 action-icon view-icon" onClick={() => handleViewClick(value)} />
            ...
            <Modal centered show={showAddModal} onHide={() => { setShowAddModal(false); resetFormFields() }} size='xl'>
                <div className="bg-light">
                    <Modal.Header closeButton>
                        <Modal.Title>Add Issue Renew</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form onSubmit={handleSubmit}>
                            <Row className="mb-3">
                                <Form.Group as={Col}>
                                    <Form.Label>Issue Renew No</Form.Label>
                                    <Form.Control
                                        placeholder="Issue renew number"
                                        type="text"
                                ...
                                <Form.Group as={Col}>
                                    <Form.Label>Issue Renew Date</Form.Label>
                                    <Form.Control
                                        type="date"
            ...
            <Modal centered show={showDeleteModal} onHide={() => { setShowDeleteModal(false); resetFormFields() }}>
                ...
                <Modal.Body>
                    Are you sure you want to delete this issue renew?
                </Modal.Body>
                <Modal.Footer>
            ...
            <Modal centered show={showViewModal} onHide={() => setShowViewModal(false)} size='xl'>
                <div className="bg-light">
                    <Modal.Header closeButton>
                        <Modal.Title>View Issue Renew</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        {viewDetails && (
                            <Form>
